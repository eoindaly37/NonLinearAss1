...
public class myBinarySearchTreeImpl<T1 extends Comparable<? super T1>, T2> implements myBinarySearchTree<T1, T2> {
	...
	public int my_count_at_level(int level){
		...
		if (this.my_is_empty()==true) {
			...
			myBinarySearchTree<T1, T2> myNewRightSubtree = this.my_right_tree();
			myBinarySearchTree<T1, T2> myNewLeftSubtree = this.my_left_tree();
			
			res = (myNewRightSubtree.my_count_at_level(level-1) + myNewLeftSubtree.my_count_at_level(level-1));
	...
	public boolean my_is_balanced(){
		...
		if(this.my_is_empty()==true) {
			...
			int rightlength = myNewRightSubtree.my_length();
			int leftlength = myNewLeftSubtree.my_length();
			...
			if((rightlength-leftlength)>1) {
				...
				res = myNewLeftSubtree.my_is_balanced();
				if(res==true) {
					res = myNewRightSubtree.my_is_balanced();
				}
				else {
					res = false;
				}
	...
	public int my_count_smaller_nodes(T1 key){
		...
		if(this.my_is_empty()==true) {
		...
		else if(this.my_root().getKey().compareTo(key)==-1) {
			res++;
			...
			res += this.my_left_tree().my_count_smaller_nodes(key);
			//WHEN THE ROOT IS NOT BIGGER, ONLY GO LEFT BECAUSE ITS SMALLER IN BINARY TREES
		}
	...
	public int my_find_node_at_level(T1 key){
		...
		if(this.my_is_empty()==true) {
			...
			res++;
			if(this.my_root().getKey().compareTo(key)==0) {
				//do nothing
			}
			else if(this.my_root().getKey().compareTo(key)==1) {
